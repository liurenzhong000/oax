package com.oax.common.util.excel;
...
public class ExcelUtils {
    ...
    private static void setDataList(SXSSFWorkbook sxssfWorkbook, SXSSFRow sxssfRow, List<List<String[]>> dataLists, HashMap notBorderMap,
        ...
        for (List<String[]> list : dataLists) {
            ...
            if (labelName != null) {
                ...
                setExcelStyles(cellStyle, cell, sxssfWorkbook, sxssfRow, 16, true, true, false, false, false, null, 399);
                jRow = 1;
            }
            ...
            for (String[] listValue : list) {
                ...
                for (int j = 0; j < listValue.length; j++) {
                    ...
                    columnIndex++;
                    //  所有单元格默认样式。
                    setExcelStyles(cellStyle, notBorderMap, cell, sxssfWorkbook, sxssfRow, k, jRow);
                    //  自定义：每个表格每一列的样式（看该方法说明）。
                    if (columnStyles != null && jRow >= pane) {
                        setExcelCellStyles(cellStyle, cell, sxssfWorkbook, sxssfRow, (List) columnStyles.get(k + 1), j);
                    }
                    //  自定义：每个表格每一行的样式（看该方法说明）。
                    if (rowStyles != null) {
                        setExcelCellStyles(cellStyle, cell, sxssfWorkbook, sxssfRow, (List) rowStyles.get(k + 1), jRow);
                    }
                    //  自定义：每一个单元格样式（看该方法说明）。
                    if (styles != null) {
                        setExcelStyles(cellStyle, cells, sxssfWorkbook, sxssfRow, (List<List<Object[]>>) styles.get(k + 1), j, jRow);
                    }
                }
                jRow++;
            }
     ...
     * @param jRow
     */
    private static void setExcelStyles(CellStyle cellStyle, HashMap notBorderMap, Cell cell, SXSSFWorkbook wb, SXSSFRow sxssfRow, int k, int jRow) {
        Boolean border = true;
        ...
        setExcelStyles(cellStyle, cell, wb, sxssfRow, null, null, true, border, false, false, null, null);
    }
     ...
     * @param height       行高
     */
    private static void setExcelStyles(CellStyle cellStyle, Cell cell, SXSSFWorkbook wb, SXSSFRow sxssfRow, Integer fontSize, Boolean bold, Boolean center, Boolean isBorder, Boolean leftBoolean,
...
//        CellStyle cellStyle = wb.createCellStyle();
     ...
     * @param rowIndex
     */
    private static void setExcelCellStyles(CellStyle cellStyle, Cell cell, SXSSFWorkbook wb, SXSSFRow sxssfRow, List<Object[]> rowstyleList, int rowIndex) {
        if (rowstyleList != null && rowstyleList.size() > 0) {
            ...
            for (int i = 0; i < rowstyle.length; i++) {
                if (rowIndex == rowstyle[i]) {
                    ...
                    setExcelStyles(cellStyle, cell, wb, sxssfRow, fontSize, Boolean.valueOf(bool[3]), Boolean.valueOf(bool[0]), Boolean.valueOf(bool[4]), Boolean.valueOf(bool[2]), Boolean.valueOf(bool[1]), fontColor, height);
                }
            }
        }
    }
     ...
     * @param styles
     */
    private static void setExcelStyles(CellStyle cellStyle, Cell cell, SXSSFWorkbook wb, SXSSFRow sxssfRow, List<List<Object[]>> styles, int cellIndex, int rowIndex) {
        if (styles != null) {
