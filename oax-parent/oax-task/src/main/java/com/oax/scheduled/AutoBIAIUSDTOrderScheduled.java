 ...
 * @Description： BIAI/USDT每1分钟自动刷单,线上版本
 */
/*
@Slf4j
@Controller
public class AutoBIAIUSDTOrderScheduled {
...
/**
    ...
    public void autoMakeOrder() {
         ...
         String oaxApiKey="XBTC2018BIAI";

         String oaxApiSecret="XBTC2018242648";
        //下单数量下限
        int startNumber = 75;
        //下单数量上限
        int endNumber = 150;
        //价格下限,盘低
        double startPrice = 0.0324;
        //价格上限，盘高
        double endPrice = 0.0358;
        //价格精度
        ...
        try{
            if (random.nextBoolean()) {//true,下降
                Trade trade = tradeService.selectByMarketId(marketId);
                List<MarketOrders> list=ordersMapper.findMarketOrdersList(marketId,1);
                ...
                //下卖单
                list=ordersMapper.findMarketOrdersList(marketId,1);
                ...
                params.put("market", tradeName);
                params.put("type", "1");
                if (new BigDecimal(endPrice).compareTo(marketOrders.getPrice().subtract(new BigDecimal(precision))) > 0) {//价格上限>交易价格
                    if (new BigDecimal(startPrice).compareTo(marketOrders.getPrice().subtract(new BigDecimal(precision))) < 0) {
                        params.put("price", marketOrders.getPrice().subtract(new BigDecimal(precision)).toString());//price=交易价格
                    }else {
                        params.put("price", new BigDecimal(startPrice).toString());//price=交易价格
                    }
                } else {
                ...
                List<MarketOrders> list=ordersMapper.findMarketOrdersList(marketId,2);
                Trade trade = tradeService.selectByMarketId(marketId);
                ...
                //下买单
                 trade = tradeService.selectByMarketId(marketId);
                 list=ordersMapper.findMarketOrdersList(marketId,2);
                 marketOrders = list.get(1);//卖2
                if (marketOrders != null) {//存在卖1单
                ...
                params.put("market", tradeName);
                params.put("type", "2");
                if (new BigDecimal(endPrice).compareTo(marketOrders.getPrice().add(new BigDecimal(precision))) > 0) {//价格上限>交易价格
                    if (new BigDecimal(startPrice).compareTo(marketOrders.getPrice().add(new BigDecimal(precision)))< 0) {
